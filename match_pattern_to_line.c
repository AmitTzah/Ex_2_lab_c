...
int does_str_match_the_current_line_from_offset(char * temp_pattern,const int* zero_index_of_line,char * current_line){
    int i= (*zero_index_of_line);
    int j=0;
    int is_match=1;
...
int is_match_between_line_and_current_array_element(char * current_line,regular_exp_tav*  regular_exp_tav_array,
                                                    size_t* zero_index_of_pattern, int* zero_index_of_line){
    int is_match;
    int is_match_str_1;
    int is_match_str_2;
...
int  is_match_in_place(char *current_line, regular_exp_tav* regular_exp_tav_array, size_t* zero_index_of_pattern,
                       int  zero_index_of_line, size_t size_of_array,switches* swt) {



    int is_match = is_match_between_line_and_current_array_element(current_line, regular_exp_tav_array, zero_index_of_pattern,
                                                                   &zero_index_of_line);
    ...
    if (is_match==1 && *zero_index_of_pattern==size_of_array){
        ...
        return is_match_in_place(current_line, regular_exp_tav_array, zero_index_of_pattern,
                                 zero_index_of_line,size_of_array,swt);
    }
...
void match_in_case_e(char *current_line, regular_exp_tav* regular_exp_tav_array, size_t size_of_array, int* is_match,
    ...
    int zero_index_of_line=0;

    while((*current_line != '\n') && (*current_line != '\0') && *is_match==0) {
        zero_index_of_pattern=0;
        *is_match = is_match_in_place(current_line,regular_exp_tav_array,&zero_index_of_pattern,zero_index_of_line, size_of_array,swt );
        current_line++;
    }
